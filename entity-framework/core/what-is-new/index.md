---
title: Novedades de EF Core - EF Core
author: divega
ms.date: 02/20/2018
ms.assetid: C21F89EE-FB08-4ED9-A2A0-76CB7656E6E4
uid: core/what-is-new/index
ms.openlocfilehash: 45b3418b15e00e5b83fb7327907dd95b76928692
ms.sourcegitcommit: b3c2b34d5f006ee3b41d6668f16fe7dcad1b4317
ms.translationtype: HT
ms.contentlocale: es-ES
ms.lasthandoff: 11/15/2018
ms.locfileid: "51688581"
---
# <a name="what-is-new-in-ef-core"></a>Novedades de EF Core

Puede usar los siguientes vínculos para obtener información sobre las nuevas características de cada versión:

## <a name="future-releases"></a>Próximas versiones

- [Guía básica de EF Core](xref:core/what-is-new/roadmap)

## <a name="recent-releases"></a>Versiones recientes

- [EF Core 2.2 (en versión preliminar)](xref:core/what-is-new/ef-core-2.2)
- [EF Core 2.1 (versión estable más reciente)](xref:core/what-is-new/ef-core-2.1)

## <a name="past-releases"></a>Versiones anteriores

- [EF Core 2.0](xref:core/what-is-new/ef-core-2.0)
- [EF Core 1.1](xref:core/what-is-new/ef-core-1.1)
- [EF Core 1.0](xref:core/what-is-new/ef-core-1.0)
